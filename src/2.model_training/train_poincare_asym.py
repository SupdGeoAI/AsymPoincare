...
from utils.model_poincare_asym import PoincareManifold, DistanceEnergyFunction, RiemannianSGD
    ...
    def calc_neg_prob(self, window_size):
        ...
        for sequence in self.sequenceList:
            ...
            for i in range(seq_len-1):
                ...
                for j in neighbors:
                    if (sequence[i] < 1000000 and j > 1000000) or (sequence[i] > 1000000 and j < 1000000):
            ...
            for j in range(len(self.word2id)):
                if i != j:
                    ...
                    if len(low_counts_pos) != 0:
                        ...
                        label = low_counts_pos
                        prob_pernode[j] = {'prob':low_counts,'word':label}
            self.probs[i] = prob_pernode

    def getNegatives(self,u,v,nneg):
        probs = np.array(self.probs[u][v]['prob'])
        labels = np.array(self.probs[u][v]['word'])
        tmp = np.random.choice(labels, size=nneg, p=probs)
        return tmp
...
class PoincareDataset(Dataset):
    ...
    def create_sample(self):
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(self.process_sequence, sequenceList[i:i+batch_size])
                for i in range(0, len(sequenceList), batch_size)
            ]
            for future in tqdm(futures, desc="Sampling Negative Samples"):
        ...
        for sequence in sequences:
            ...
            for i, u in enumerate(word_ids):
                for j, v in enumerate(word_ids[(i + 1):(i + self.window_size + 1)]):
                    if (u != v) and (v in self.data.probs[u]):
                        negatives = self.data.getNegatives(u, v, self.nneg)
                        samples.append((u, v, negatives))
        return samples
...
class PoincareEmbedding:
    ...
    def train(self):
        ...
        for wid, w in self.data.id2word.items():
            if int(w) < 1000000:
                value_to_find = str(int(w) + 1000000)
                exit_node_id = next((k for k, y in self.data.id2word.items() if y == value_to_find), None)
                entry_ids_list.append(wid)
                exit_ids_list.append(exit_node_id)

        lr = self.lr
        loss_ratio = self.r
        save_log_path = self.save_dir + '/logs.txt'
        ...
        for epoch in range(self.epochs):
            ...
            for i, sample_batched in enumerate(tqdm(self.dataloader, desc="Batch Training")):
                ...
                dist_sum, preds = self.model(inputs, entry_ids_list, exit_ids_list)
                loss = loss_ratio*self.model.loss(preds, targets) + (1-loss_ratio)*dist_sum
                loss.backward()
            ...
            self.model.save_embedding(epoch, self.data.id2word, self.save_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nneg', type=int, default=50)
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--window_size', type=int, default=5)
    parser.add_argument('--epsilon', type=float, default=1e-5)
    parser.add_argument('--batch_size', type=int, default=1280)
    parser.add_argument('--emb_dimension', type=int, default=2)
    parser.add_argument('--lr_exp_scheduler', type=float, default=0.96)
    args = parser.parse_args()
    
    argsDict = args.__dict__
    
    save_dir = '../../logs/v_' + str(args.version)
    ...
    model = PoincareEmbedding(
        ...
        lr_exp_scheduler=args.lr_exp_scheduler
    )
