...
class EnergyFunction(torch.nn.Module):
    ...
    def forward(self,inputs,entry_ids_list,exit_ids_list):
        device = self.lt.weight.device
        entry_ids_list = torch.tensor(entry_ids_list, dtype=torch.long, device = device)
        exit_ids_list = torch.tensor(exit_ids_list, dtype=torch.long, device = device)
        list_entry = self.lt.weight[entry_ids_list]
        list_exit = self.lt.weight[exit_ids_list]
        ...
        combined_list_entry_s = torch.cat([list_entry, s.reshape(-1, s.size(-1))],dim=0)
        combined_list_exit_o = torch.cat([list_exit, o.reshape(-1, o.size(-1))],dim=0)
        combined_distances = self.energy(combined_list_entry_s,combined_list_exit_o)

        dist_sum = combined_distances[:list_entry.size(0)].sum()
        forward_dist = combined_distances[list_entry.size(0):].view(s.size(0), s.size(1), 1).squeeze(-1)

        return dist_sum, forward_dist
...
class InvalidPoincareDistanceError(Exception):
    pass

class DistanceEnergyFunction(EnergyFunction):
    def __init__(self, manifold, dim, size, eps=1e-5, sparse=False, **kwargs):
        super(DistanceEnergyFunction, self).__init__(manifold, dim, size, sparse, **kwargs)
        self.eps = eps
