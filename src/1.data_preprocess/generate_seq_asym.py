...
def getTransitionMatrix(G, nodes, epsilon=1e-5):
    ...
    size = len(nodes)
    matrix = np.zeros((size, size)) + epsilon
...
def generateAsymSequence(startIndex, transitionMatrix, path_length, alpha):
    size = len(transitionMatrix[0])
    result = [startIndex, startIndex+size]
    current = startIndex
    for i in range(0, path_length):
        if random.random() < alpha:
            ...
            nextIndex = np.random.choice(len(probs), size=1, p=probs)[0]
        result.append(nextIndex)
        result.append(nextIndex+size)
        current = nextIndex

    return result

def random_walk_asym(G, num_paths, path_length, alpha):
    nodes = list(G.nodes())
    ...
    for i in tqdm(range(0, size), desc="Generating Sequences"):
        for j in range(0, num_paths):
            indexList = generateAsymSequence(i, transitionMatrix, path_length, alpha)
            sentence=[]
            for index in indexList:
                if index<size:
                    node=int(nodes[index])
                else:
                    node=int(nodes[index-size])+1000000
...
if __name__ == '__main__':
    ...
    sentenceList = random_walk_asym(graph, args.num_paths, args.path_length, args.alpha)
